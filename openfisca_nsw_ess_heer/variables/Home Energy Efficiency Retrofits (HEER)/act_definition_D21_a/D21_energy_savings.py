...
class D21_a_deemed_activity_electricity_savings(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What are the deemed activity electricity savings for Activity' \
            ' Definition D21a (version with VEU registry integration)?'
    ...
    def formula(buildings, period, parameters):
        ...
        electricial_energy = buildings('D21_a_annual_electrical_energy', period)
        baseline_A = parameters(period).HEER.D21.table_D21_a_baseline_A_and_B.baseline_A[heat_pump_system_size]
        coefficient_b = parameters(period).HEER.D21.table_D21_a_coefficients.coefficient_b
        electricity_savings_factor = (baseline_A - coefficient_b * electricial_energy)
        return electricity_savings_factor


class D21_a_deemed_activity_gas_savings(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What are the deemed activity electricity savings for Activity' \
    ...
    def formula(buildings, period, parameters):
        ...
        heat_pump_system_size = buildings('heat_pump_system_size', period)
        HeatPumpSystemSize = heat_pump_system_size.possible_values  # imports functionality of heat pump system size enum from user_inputs
        supplementary_energy = buildings('D21_a_annual_supplementary_energy', period)
        baseline_B = parameters(period).HEER.D21.table_D21_a_baseline_A_and_B.baseline_B[heat_pump_system_size]
        coefficient_a = parameters(period).HEER.D21.table_D21_a_coefficients.coefficient_a
        gas_savings_factor = (baseline_B - (coefficient_a * supplementary_energy))
        return gas_savings_factor


class D21_a_annual_supplementary_energy(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class D21_a_annual_electrical_energy(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
