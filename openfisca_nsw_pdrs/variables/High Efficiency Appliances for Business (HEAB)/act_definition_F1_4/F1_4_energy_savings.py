...
class F1_4_electricity_savings(Variable):
    ...
    def formula(buildings, period, parameters):
        ...
        m_coefficient = parameters(period).HEAB.F1.F1_4_coefficients.M[product_class]
        n_coefficient = parameters(period).HEAB.F1.F1_4_coefficients.N[product_class]
...
class F1_4_daily_total_electricity_consumption(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class F1_4_total_display_area(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
