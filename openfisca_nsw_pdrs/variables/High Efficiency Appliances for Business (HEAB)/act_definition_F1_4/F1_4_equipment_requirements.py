...
class F1_4_is_integral_RDC(Variable):
    value_type = bool
    entity = Building
...
class F1_4_is_registered_in_GEMS_2012(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
...
class F1_4_is_registered_in_GEMS_2019(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
    label = 'Is the End User Equipment a registered product based on Greenhouse' \
...
class F1_4_annual_energy_consumption_below_baseline_EEI(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
    ...
    def formula(buildings, period, parameters):
        annual_energy_consumption = buildings('F1_4_annual_energy_consumption', period)
        product_class = buildings('F1_product_class', period)
        F1ProductClass = product_class.possible_values
        EEI = parameters(period).HEAB.F1.F1_4_baseline_EEIs.EEI[product_class]
        return annual_energy_consumption < EEI


class F1_4_annual_energy_consumption(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class F1_4_meets_all_equipment_requirements(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
    ...
    def formula(buildings, period, parameters):
        is_integral_RDC = buildings('F1_4_is_integral_RDC', period)
        registered_in_GEMS_2012 = buildings('F1_4_is_registered_in_GEMS_2012', period)
        registered_in_GEMS_2019 = buildings('F1_4_is_registered_in_GEMS_2019', period)
        below_baseline_EEI = buildings('F1_4_annual_energy_consumption_below_baseline_EEI')
