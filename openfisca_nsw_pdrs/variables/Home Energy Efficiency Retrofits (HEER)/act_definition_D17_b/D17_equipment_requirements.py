...
class D17_b_is_listed_on_product_register(Variable):
    value_type = bool
    entity = Building
...
class D17_b_is_certified_to_AS_NZS_2712(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
...
class D17_b_certified_to_60_percent_savings_in_zone_HP3_AU(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
    label = 'Is the equipment certified to have performance of at least 60%' \
            ' savings when modelled in climate zone HP3_AU?'
    # need to figure out logic for climate zones


class D17_b_meets_all_equipment_requirements(Variable):
    value_type = bool
    entity = Building
    definition_period = ETERNITY
    ...
    def formula(buildings, period, parameters):
        listed_on_product_register = buildings('D17_b_is_listed_on_product_register', period)
        product_is_certified_to_standard = buildings('D17_b_is_certified_to_AS_NZS_2712', period)
        ...
        certified_to_60_percent_savings_in_HP3 = buildings('D17_b_certified_to_60_percent_savings_in_zone_HP3_AU', period)
