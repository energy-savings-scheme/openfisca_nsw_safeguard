...
class D4_new_electricity_savings_factor(Variable):
    value_type = float
    entity = Building
    ...
    def formula(buildings, period, parameters):
        ...
        electricity_savings_factor = (parameters(period).table_D4.electricity_savings_factor[star_rating])
        return electricity_savings_factor


class D4_new_reference_cooling_annual_energy_use(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What is the reference cooling annual energy use for the AC, as' \
            ' defined in Table D4_new.4?'

    def formula(buildings, period, parameters):
        cooling_capacity = buildings('cooling_capacity', period)
        weather_zone = buildings('weather_zone', period)
        cooling_hours = parameters(period).HEAB.D4_new.cooling_and_heating_hours.cooling_hours[weather_zone]
        product_class = buildings('D4_new_product_class', period)
        baseline_cooling_AEER = parameters(period).HEAB.D4_new.baseline_AEER_and_ACOP.AEER[product_class]
        return cooling_capacity * cooling_hours / baseline_cooling_AEER


class D4_new_cooling_capacity(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What is the cooling capacity for Activity Definition D4_new?'


class D4_new_reference_heating_annual_energy_use(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    ...
    def formula(buildings, period, parameters):
        ...
        weather_zone = buildings('weather_zone', period)
        heating_hours = parameters(period).HEAB.D4_new.cooling_and_heating_hours.heating_hours[weather_zone]
        product_class = buildings('D4_new_product_class', period)
        baseline_heating_ACOP = parameters(period).HEAB.D4_new.baseline_AEER_and_ACOP.ACOP[product_class]
        return heating_capacity * heating_hours / baseline_heating_ACOP


class D4_new_heating_capacity(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What is the heating capacity for Activity Definition D4_new?'


class D4_new_cooling_annual_energy_use(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class D4_new_cooling_power_input(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What is the cooling power input for Activity Definition D4_new?'


class D4_new_heating_annual_energy_use(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    ...
    def formula(buildings, period, parameters):
        heating_power_input = buildings('D4_new_heating_power_input', period)
        weather_zone = buildings('D4_new_weather_zone', period)
        heating_hours = parameters(period).HEAB.D4_new.cooling_and_heating_hours.heating_hours[weather_zone]
        heating_annual_energy_use = heating_power_input * heating_hours
        return heating_annual_energy_use


class D4_new_heating_power_input(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'What is the heating power input for Activity Definition D4_new?'


class D4_new_WeatherZone(Enum):
...
class D4_new_weather_zone(Variable):
    value_type = Enum
    possible_values = D4_new_WeatherZone
    default_value = D4_new_WeatherZone.hot_zone
    entity = Building
    definition_period = ETERNITY
...
class D4_new_ProductClass(Enum):
...
class D4_new_product_class(Variable):
    value_type = Enum
    possible_values = D4_new_ProductClass
    default_value = D4_new_ProductClass.product_class_one
    entity = Building
    definition_period = ETERNITY
