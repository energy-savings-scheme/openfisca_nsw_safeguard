...
class E1_residential_savings_factor(Variable):
    value_type = float
    entity = Building
    ...
    def formula(buildings, period, parameters):
        ...
        residential_building_savings_factor = (parameters(period).table_E1_1.residential_savings_factor
        [existing_lamp_type][new_lamp_type][lamp_rating_power])
        return residential_building_savings_factor


class E1_small_business_savings_factor(Variable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    label = 'Returns the relevant savings factor for the existing and new lamp' \
