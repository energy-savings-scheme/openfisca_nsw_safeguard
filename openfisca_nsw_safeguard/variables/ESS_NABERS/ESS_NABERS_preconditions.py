...
# today is used to calculate the age of the Historical Rating
...
class ESS__NABERS_current_rating_year(Variable):
    ...
    def formula(buildings, period, parameters):
        end_date_of_current_NABERS_rating_period = buildings(
...
class ESS__NABERS_historical_rating_year(Variable):
    ...
    def formula(buildings, period, parameters):
        end_date_of_historical_NABERS_rating_period = buildings(
...
class ESS__NABERS_is_eligible_for_method_one(Variable):
    ...
    def formula(buildings, period, parameters):
        is_first_NABERS_rating = buildings(
            'ESS__NABERS_first_NABERS_rating', period)
        obtained_for_legal_requirement = buildings(
            'ESS__NABERS_rating_obtained_for_legal_requirements', period)
        exceeds_benchmark = buildings(
            'ESS__NABERS_exceeds_A20_benchmark_rating_by_half_star', period)
...
class ESS__NABERS_no_more_than_7_years_between_current_year_and_historical_rating_date(Variable):
    ...
    def formula(buildings, period, parameters):
        ...
        current_rating_year = end_date_of_current_NABERS_rating_period.astype(
            'datetime64[Y]')
        end_date_of_current_rating_year = (current_rating_year + np.timedelta64(
            11, 'M') + np.timedelta64(30, 'D')).astype('datetime64[D]')
        ...
        maximum_number_of_years_between_historical_and_current = parameters(
            period).ESS.MBM.NABERS.NABERS_related_constants.years_between_historical_and_current_rating
...
class ESS__NABERS_implementation_date(Variable):
    ...
    def formula(buildings, period, parameters):
        current_rating_end_date = buildings(
            'ESS__NABERS_end_date_of_current_NABERS_rating_period', period)
...
class ESS__NABERS_is_eligible(Variable):
    ...
    def formula(buildings, period, parameters):
        ...
        has_GreenPower = buildings(
            'ESS__NABERS_building_has_GreenPower', period)
        on_site_generation_identified = buildings(
