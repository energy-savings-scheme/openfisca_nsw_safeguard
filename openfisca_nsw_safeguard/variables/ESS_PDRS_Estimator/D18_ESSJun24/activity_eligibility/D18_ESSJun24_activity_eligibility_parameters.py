...
class D18_ESSJun24_equipment_replaced(Variable):
    ...
    metadata = {
      'display_question' : 'Is the activity the replacement of an existing electric resistance storage or instantaneous water heater with a solar (electric boosted) water heater?',
      'sorting' : 1,
      'eligibility_clause' : """A new installation is not eligible under ESS Activity D18. In ESS D18 the activity definition states that the activity must be the replacement of an existing electric water heater with an (air source) heat pump water heater. <br />
                                ESS D18 Eligibility Requirements Clause 1 also states that the existing electric water heater must be an electric resistance storage or instantaneous water heater."""
    }
...
class D18_ESSJun24_minimum_payment(Variable):
    ...
    metadata = {
      ...
      'eligibility_clause' : """In ESS Clause 9.8.1(F) it states that the Accredited Certificate Provider must have evidence satisfactory to the Scheme Administrator that the Purchaser has paid for the Implementation, assessment and other associated works carried out at the Site a Net Amount of:<br /> 
                                i. at least $200 (excluding GST) for each item of End-User Equipment installed as part of an Implementation using any of Activity Definitions D5, D16, D18, D18, D19, D20 and D21;"""
    }
