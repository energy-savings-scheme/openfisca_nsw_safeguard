...
""" Parameters for D18_ESSJun24 ESC Calculation
...
class D18_ESSJun24_get_HP_zone_by_BCA_climate_zone(Variable): 
    ...
    def formula(building, period, parameters):
        BCA_climate_zone = building('D18_ESSJun24_BCA_climate_zone_by_postcode_int', period)
...
class D18_ESSJun24_system_size(Variable):
    ...
    entity = Building
    definition_period = ETERNITY
    possible_values = D18_ESSJun24_System_Size
    default_value = D18_ESSJun24_System_Size.system_size_small
    metadata = {
      'variable-type': 'user-input',
      'label': 'System Size',
...
class D18_ESSJun24_Baseline_A(Variable):
    ...
    def formula(buildings, period, parameters):
        system_size = buildings('D18_ESSJun24_system_size_int', period)

        baseline_A = parameters(period).ESS.HEER.table_D18_1_ESSJun24['baseline_energy_consumption'][system_size]
        return baseline_A
