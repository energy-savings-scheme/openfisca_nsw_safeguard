...
class D19_ESSJun24_equipment_replaced(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    ...
    metadata = {
      ...
      'sorting' : 1,
      'eligibility_clause' : """A new installation is not eligible under ESS Activity D19. In ESS D19 the activity definition states that the activity must replace an existing gas water heater with an (air source) heat pump water heater. <br />
                                ESS D19 Eligibility Requirements Clause 1 also states that the existing gas water heater must be a gas storage or instantaneous water heater."""
    }


class D19_ESSJun24_engaged_ACP(BaseVariable):    
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class D19_ESSJun24_minimum_payment(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class D19_ESSJun24_equipment_removed(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class D19_ESSJun24_equipment_installed(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class D19_ESSJun24_installed_by_qualified_person(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class D19_ESSJun24_equipment_registered_IPART(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
    metadata = {
        ...
        'eligibility_clause' : """In order to be listed on the IPART product register, the product has met the following three product requirements of this activity:<br /> 
                                  1.The installed End-User Equipment must be an air source heat pump water heater as defined in AS/NZS 4234;<br />
                                  2.The installed End-User Equipment must be certified to AS/NZS 2712; and <br />
                                  3.The installed End-User equipment must achieve minimum annual energy savings, when determined as an air sourced heat pump using a small or medium thermal peak load System Size in accordance with AS/NZS 4232, of: <br />
                                    • 60% when modelled in climate zone HP3-AU<br />
                                    • 60% when modelled in climate zone HP5-AU"""
    }


class D19_ESSJun24_split_system(BaseVariable):
    value_type = bool
    entity = Building
    default_value = False
    definition_period = ETERNITY
...
class D19_ESSJun24_safety_requirement(BaseVariable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
