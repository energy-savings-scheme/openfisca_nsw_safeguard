...
class F17_ESSJun24_equipment_new_installation(Variable):
    value_type = bool
    entity = Building
    default_value = True
...
class F17_ESSJun24_4234_certified(Variable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class F17_ESSJun24_installed_by_qualified_person(Variable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class F17_ESSJun24_engaged_ACP(Variable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class F17_ESSJun24_minimum_payment(Variable):
    value_type = bool
    entity = Building
    default_value = True
    definition_period = ETERNITY
...
class F17_ESSJun24_building_BCA_not_class_1_or_4(Variable):
    value_type = bool
    entity = Building
    default_value = False
    definition_period = ETERNITY
    metadata = {
      ...
      'sorting' : 6,
      'eligibility_' : """In ESS F17 Eligibility Requirements Clause 1 it states that the New End-User Equipment must not be installed in a BCA Class 1 or 4 building."""
    }


class F17_ESSJun24_StorageVolume(Enum):
    less_than_or_equal_to_700_L = 'Less than or equal to 700 litres'
    more_than_700_L = 'More than 700 litres'


class F17_ESSJun24_storage_volume(Variable):
    value_type = Enum
    entity = Building
    default_value = F17_ESSJun24_StorageVolume.more_than_700_L
    possible_values = F17_ESSJun24_StorageVolume
    definition_period = ETERNITY
    metadata = {
...
class F17_ESSJun24_storage_volume_int(Variable):
    value_type = int
    entity = Building 
    definition_period = ETERNITY

    def formula(buildings, period, parameters):
      storage_volume = buildings('F17_ESSJun24_storage_volume', period)
      ...
      [
        F17_ESSJun24_StorageVolume.less_than_or_equal_to_700_L,
        F17_ESSJun24_StorageVolume.more_than_700_L
      ])

      return storage_volume_int


class F17_ESSJun24_certified(Variable):
...
class F17_ESSJun24_equipment_certified_by_storage_volume(Variable):
    ...
    def formula(buildings, period, parameters):
      storage_volume = buildings('F17_ESSJun24_storage_volume', period)
      certified_AS_NZ_2712 = buildings('F17_ESSJun24_certified', period)

      eligible_by_storage = np.select(
        [
          (storage_volume == F17_ESSJun24_StorageVolume.less_than_or_equal_to_700_L) * certified_AS_NZ_2712,
          (storage_volume == F17_ESSJun24_StorageVolume.less_than_or_equal_to_700_L) * np.logical_not(certified_AS_NZ_2712),
          (storage_volume == F17_ESSJun24_StorageVolume.more_than_700_L)
        ],
        [
          True,
          False,
