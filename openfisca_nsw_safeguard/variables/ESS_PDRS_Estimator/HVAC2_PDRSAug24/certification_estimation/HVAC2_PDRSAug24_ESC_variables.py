...
class HVAC2_PDRSAug24_baseline_AEER_input(BaseVariable):
    ...
    entity = Building
    definition_period = ETERNITY
    metadata = {"alias": "AEER", "variable-type": "output", "label": "Baseline AEER"}
    ...
    def formula(building, period, parameters):
        ...
        baseline_aeer = parameters(period).ESS.HEAB.table_F4_2_product_class.AEER[
            new_or_replacement_activity
        ][product_class]
...
class HVAC2_PDRSAug24_get_climate_zone_by_postcode(BaseVariable):
    ...
    entity = Building
    definition_period = ETERNITY
    metadata = {"variable-type": "inter-interesting", "alias": "climate zone"}

    def formula(building, period, parameters):
        postcode = building("HVAC2_PDRSAug24_PDRS__postcode", period)
        rnf = parameters(period).ESS.ESS_general.table_A27_4_climate_zone_by_postcode
        zone_int = rnf.calc(postcode)
        climate_zone_str = np.select(
            [zone_int == 1, zone_int == 2, zone_int == 3], ["hot", "mixed", "cold"]
        )
...
class HVAC2_PDRSAug24_equivalent_heating_hours_input(BaseVariable):
    ...
    entity = Building
    definition_period = ETERNITY
    metadata = {"variable-type": "output"}

    def formula(building, period, parameters):
        climate_zone = building("HVAC2_PDRSAug24_certificate_climate_zone", period)
        climate_zone_str = np.select(
            [climate_zone == 1, climate_zone == 2, climate_zone == 3],
            ["hot_zone", "average_zone", "cold_zone"],
        )
        heating_hours = parameters(period).ESS.HEAB.table_F4_1.equivalent_heating_hours[
            climate_zone_str
        ]
...
class HVAC2_PDRSAug24_equivalent_cooling_hours_input(BaseVariable):
    ...
    def formula(building, period, parameters):
        ...
        cooling_hours = parameters(period).ESS.HEAB.table_F4_1.equivalent_cooling_hours[
            climate_zone_str
        ]
...
class HVAC2_PDRSAug24_baseline_ACOP_input(BaseVariable):
    ...
    def formula(building, period, parameters):
        product_class = building("HVAC2_PDRSAug24_product_class", period)
        new_or_replacement_activity = building("HVAC2_PDRSAug24_Activity", period)
        baseline_acop = parameters(period).ESS.HEAB.table_F4_2_product_class.ACOP[
            new_or_replacement_activity
        ][product_class]
...
class HVAC2_PDRSAug24_TCSPF_or_AEER_exceeds_ESS_benchmark(BaseVariable):
    ...
    entity = Building
    definition_period = ETERNITY
    metadata = {"alias": "Air Conditioner has at least 5 years of Warranty"}
    ...
    def formula(buildings, period, parameters):
        ...
        AC_exceeds_cooling_benchmark = np.where(
            ...
            (
                AC_AEER
                >= parameters(period).PDRS.AC.table_HVAC_2_2_product_class["AEER"][
                    product_class
                ]
            ),
            (
                AC_TCSPF
                >= parameters(period).PDRS.AC.table_HVAC_2_2_product_class[
                    "TCSPF_mixed"
                ][product_class]
            ),
        )
...
class HVAC2_PDRSAug24_HSPF_or_ACOP_exceeds_ESS_benchmark(BaseVariable):
    ...
    entity = Building
    definition_period = ETERNITY
    metadata = {"alias": "ESS - HSPF or ACOP exceeds benchmark"}
    ...
    def formula(buildings, period, parameters):
        ...
        HSPF_is_zero = (AC_HSPF == 0) + (AC_HSPF == None)
        ...
        AC_exceeds_benchmark = np.select(
            ...
            [
                (
                    AC_ACOP
                    >= parameters(period).ESS.HEAB.table_F4_4_product_class["ACOP"][
                        product_class
                    ]
                ),
                (
                    AC_HSPF
                    >= parameters(period).ESS.HEAB.table_F4_4_product_class[
                        "HSPF_cold"
                    ][product_class]
                ),
                (
