...
class SYS1_gas_savings(Variable):
    ...
    def formula(buildings, period, parameters):
        gas_savings =    
        gas_savings_conversion_factor = 0.39

        gas_savings = (gas_savings * gas_savings_conversion_factor)
        return gas_savings
...
class SYS1_electricity_savings(Variable):
    ...
    entity = Building
    definition_period = ETERNITY
    label = 'SYS1 Electricity savings'
    metadata = {
        "alias": "HVAC1 electricity savings",
        "variable-type": "inter-interesting"
    }
    ...
    def formula(buildings, period, parameters):
        electricity_savings = 
        electricity_savings_conversion_factor = 1.06

        electricity_savings = (electricity_savings * electricity_savings_conversion_factor)
...
class SYS1_ESC_calculation(Variable):
    ...
    def formula(buildings, period, parameters):
      SYS1_electricity_savings = buildings("SYS1_electricity_savings", period)
      SYS2_gas_savings = buildings("SYS2_gas_savings", period)
      result_to_return = np.sum(SYS1_electricity_savings, SYS2_gas_savings)
