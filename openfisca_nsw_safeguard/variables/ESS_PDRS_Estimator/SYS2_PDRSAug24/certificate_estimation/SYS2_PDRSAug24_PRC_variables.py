...
class SYS2_PDRSAug24_pool_size(Variable):
    value_type = Enum
    entity = Building
...
class SYS2_PDRSAug24_pool_size_int(Variable):
    value_type = str
    entity = Building
    definition_period = ETERNITY
...
class SYS2_PDRSAug24_baseline_input_power(Variable):
    ...
    def formula(buildings, period, parameters):
        pool_size = buildings('SYS2_PDRSAug24_pool_size_int', period)

        baseline_input_power = parameters(period).PDRS.pool_pumps.table_SYS2_PDRSAug24_1['baseline_input_power'][pool_size]
...
class SYS2_PDRSAug24_pool_pump_type(Variable):
    value_type = Enum
    entity = Building
...
class SYS2_PDRSAug24_input_power(Variable):
    ...
    def formula(buildings, period, parameters):
        ...
        input_power = parameters(period).PDRS.pool_pumps.table_SYS2_PDRSAug24_2['input_power'][pool_size][star_rating][pool_pump_type]
        return input_power
