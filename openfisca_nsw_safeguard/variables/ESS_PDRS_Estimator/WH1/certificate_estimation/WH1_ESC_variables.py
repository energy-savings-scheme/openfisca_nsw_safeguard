...
"""
class WH1_HP_capacity_factor(BaseVariable):
...
class WH1_WH_capacity_factor(BaseVariable):
    reference = 'unit in kW'
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH1_com_peak_load(BaseVariable):
...
class WH1_HP_elec(BaseVariable):
    reference = 'Gj per year'
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH1_regional_network_factor(BaseVariable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH1_replacement_activity(BaseVariable):
    value_type = bool
    default_value = True
    entity = Building
    definition_period = ETERNITY
