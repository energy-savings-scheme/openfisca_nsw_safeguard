...
"""
class WH2_test_HP_capacity_factor(BaseVariable):
...
class WH2_test_WH_capacity_factor(BaseVariable):
    reference = 'unit in kW'
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH2_test_HP_gas(BaseVariable):
...
class WH2_test_com_peak_load(BaseVariable):
...
class WH2_test_HP_elec(BaseVariable):
    reference = 'Gj per year'
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH2_test_regional_network_factor(BaseVariable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
...
class WH2_test_replacement_activity(BaseVariable):
    value_type = bool
    default_value = True
    entity = Building
    definition_period = ETERNITY
