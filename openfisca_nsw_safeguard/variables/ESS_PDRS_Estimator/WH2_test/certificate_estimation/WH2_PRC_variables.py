...
""" Parameters for WH2_test PRC Calculation
"""

""" Values shared with ESC variables WH2_test_ESC_variables 
...
class WH2_test_annual_energy_savings(BaseVariable):
...
"""
class WH2_test_PDRS__postcode(BaseVariable):
    value_type = int
    entity = Building
    definition_period = ETERNITY
...
class WH2_test_BCA_climate_zone_by_postcode(BaseVariable):
    value_type = str
    entity = Building
    definition_period = ETERNITY
    metadata={
...
class WH2_test_BCA_climate_zone_by_postcode_int(BaseVariable):
    value_type = int
    entity = Building
    definition_period = ETERNITY
    metadata={
...
class WH2_test_get_HP_zone_by_BCA_climate_zone(BaseVariable): 
    value_type = int
    entity = Building
    definition_period = ETERNITY
    
    def formula(building, period, parameters):
        BCA_climate_zone = building('WH2_test_BCA_climate_zone_by_postcode_int', period)
...
class WH2_test_get_network_loss_factor_by_postcode(BaseVariable):
    value_type = float
    entity = Building
    definition_period = ETERNITY
    metadata = {
...
class WH2_test_annual_energy_savings_eligible(BaseVariable):
    ...
    def formula(buildings, period, parameters):
        minimum_savings = buildings('WH2_test_annual_energy_savings', period)
        heat_pump_zone = buildings('WH2_test_get_HP_zone_by_BCA_climate_zone', period)
