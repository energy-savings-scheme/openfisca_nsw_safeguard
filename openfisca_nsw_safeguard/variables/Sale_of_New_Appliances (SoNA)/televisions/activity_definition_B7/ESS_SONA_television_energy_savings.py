...
class ESS_SONA_television_electricity_savings(BaseVariable):
    value_type = float
    entity = Building
    ...
    def formula(buildings, period, parameters):
        ...
        screen_size = np.select([television_screen_size < 40,
        ((television_screen_size > 40) * (television_screen_size <= 65)),
        ((television_screen_size > 65) * (television_screen_size <= 120)),
        television_screen_size >= 120],
        ["under_40cm", "40_to_65cm", "65_to_120cm", "over_120cm"])
        deemed_equipment_electricity_savings = (parameters(period).ESS.SONA.table_B7_1.deemed_equipment_electricity_savings[star_rating][screen_size])
